...
from output_func_nominalpi import *
from cmath import *
from math import *
...
# Input Fields
def input_page_nominal():
    st.title("🎯MEDIUM TRANSMISSION LINE [Nominal-∏]")

    st.header("Input Parameters")

    MGMD = 0
    Type = st.radio("Type of system", ("Symmetrical Spacing", "Unsymmetrical Spacing"))
    if Type == "Unsymmetrical Spacing":
        D1 = st.text_input("Spacing between the phase conductors 1-2")
        D2 = st.text_input("Spacing between the phase conductors 2-3")
        D3 = st.text_input("Spacing between the phase conductors 3-1")
        if D1 and D2 and D3:
    ...
    if st.button("Calculate"):
        if isfloat(sys_vol) and isfloat(R) and isfloat(l) and isfloat(f) and isfloat(P_r) and isfloat(pf) and isfloat(N) and isfloat(k) and isfloat(d) and isfloat(d_strand):
            ...
            col1.write("Inductance per phase:")
            col1.write("Capacitance per phase:")
            col1.write("Inductive Reactance:")
            col1.write("Capacitive Reactance:")
            col1.write("A Parameter:")
            col1.write("B Parameter:")
            col1.write("C Parameter:")
            col1.write("D Parameter:")
            col1.write("Charging current sending substation:")
            col1.write("Sending voltage:")
            col1.write("Sending current:")
            col1.write("Voltage Regulation:")
            col1.write("Power Loss:")
            col1.write("Efficiency:")

            col2.write(f"{abs(results.Inductance_per_phase()):.2f}")
            col2.write(f"{abs(results.Capacitance_per_phase()):.2f}")
            col2.write(f"{abs(results.Inductive_Reactance()):.2f}")
            col2.write(f"{abs(results.Capacitive_Reactance()):.2f}")
            ...
            col2.write(f"{results.Charging_current_sending_substation():.2f}")
            col2.write(f"{abs(results.Sending_voltage()) * pow(3, 0.5) / 1000:.2f}")
            col2.write(f"{abs(results.Sending_current()):.2f}")
            col2.write(f"{results.Voltage_regulation():.2f}")
            col2.write(f"{results.Power_loss():.2f}")
