...
class output:
    ...
    def A_parameter(self):
        z = (self.R * self.l) + (self.Inductive_Reactance() *1j)
        y = pow(self.Capacitive_Reactance(), -1) *1j
        return 1 + (z * y / 2)

    def C_parameter(self):
        return (pow(self.A_parameter(), 2) - 1) / self.B_parameter()
    ...
    def Charging_current_sending_substation(self):
        return abs(self.V_R() * self.C_parameter())
    ...
    def Sending_current(self):
        ReI_s = (self.C_parameter().real * self.V_R()) + (self.D_parameter().imag * self.I_R().imag) + (self.D_parameter().real * self.I_R().real)
        ImI_s = (self.C_parameter().imag * self.V_R()) - (self.D_parameter().real * self.I_R().imag) + (self.D_parameter().imag * self.I_R().real)
        return complex(ReI_s + (ImI_s *1j))

    def Voltage_regulation(self):
        return ((abs(self.Sending_voltage()) / abs(self.A_parameter())) - self.V_R()) * 100 / self.V_R()
